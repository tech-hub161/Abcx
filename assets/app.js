...
function saveReportToLocalStorage(dateStr, data) {
    ...
    const key = `maa-tara-report-${dateStr}`;
    localStorage.setItem(key, JSON.stringify(data));
}
function getReportFromLocalStorage(dateStr) {
    return JSON.parse(localStorage.getItem(`maa-tara-report-${dateStr}`));
}
function deleteReportFromLocalStorage(dateStr) {
    localStorage.removeItem(`maa-tara-report-${dateStr}`);
}
function getAllReportDates() {
    return Object.keys(localStorage)
        .filter(k => k.startsWith('maa-tara-report-'))
        .map(k => k.replace('maa-tara-report-', ''))
        .sort((a, b) => b.localeCompare(a));
}
function clearAllReports() {
    for (const k of Object.keys(localStorage)) {
        if (k.startsWith('maa-tara-report-')) localStorage.removeItem(k);
    }
}
...
function downloadCombinedPDF(reports, fromDate, toDate) {
    ...
    const win = window.open('', '', 'width=900,height=700');
    win.document.write('<html><head><title>PDF Export</title>');
    win.document.write('<style>body{font-family:sans-serif;}table{border-collapse:collapse;width:100%;}th,td{border:1px solid #888;padding:8px;text-align:center;}th{background:#eaf1fb;}h3{margin:18px 0 6px 0;} .summary-table{margin-bottom:24px;border-radius:12px;box-shadow:0 2px 8px #ff6a0022;} .summary-table th{background:#ffe0b2;color:#d7263d;} .summary-table td{background:#f3f6fa;}</style>');
    win.document.write('</head><body>');
    ...
    // Add sum row (only one cell per column)
    win.document.write('<tr style="font-weight:bold;background:#ffe0b2;color:#d7263d;">');
